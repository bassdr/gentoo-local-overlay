...
if [[ ${PV} != *9999 ]]; then
	SRC_URI_SUFFIX="gz"
	SRC_URI_DOC_SUFFIX="xz"
	SRC_URI_KORG="https://www.kernel.org/pub/software/scm/git"
	[[ "${PV/rc}" != "${PV}" ]] && SRC_URI_KORG+='/testing'
	SRC_URI="https://github.com/microsoft/${PN}/archive/v${MY_PV}.tar.${SRC_URI_SUFFIX} -> ${MY_P}.tar.${SRC_URI_SUFFIX}
		${SRC_URI_KORG}/${PN}-manpages-${DOC_VER}.tar.${SRC_URI_DOC_SUFFIX}
		doc? (
			${SRC_URI_KORG}/${PN}-htmldocs-${DOC_VER}.tar.${SRC_URI_DOC_SUFFIX}
		)"
	[[ "${PV}" == *_rc* ]] || \
...
LICENSE="GPL-2"
SLOT="0"
IUSE="+blksha1 +curl cgi doc gnome-keyring +gpg highlight +iconv mediawiki mediawiki-experimental +nls +pcre perforce +perl +ppcsha1 +safe-directory +scalar selinux subversion tk +webdav xinetd cvs test"
...
exportmakeopts() {
	...
	if use scalar; then
		myopts+=( INCLUDE_SCALAR=YesPlease )
	fi
...
src_unpack() {
	if [[ ${PV} != *9999 ]] ; then
		...
		cd "${S}" || die
		unpack ${PN}-manpages-${DOC_VER}.tar.${SRC_URI_DOC_SUFFIX}
		if use doc ; then
			pushd "${S}"/Documentation &>/dev/null || die
			unpack ${PN}-htmldocs-${DOC_VER}.tar.${SRC_URI_DOC_SUFFIX}
			popd &>/dev/null || die
		fi
	else
		git-r3_src_unpack
